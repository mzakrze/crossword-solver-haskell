...
type FileName = [Char]
------------ Main function -----------
solve wordsFileName boardFileName = aa -- printBoard solvedBoard -- FIXME printować: wykreśloną tablice, gdzie znaleziono słowa(??), secret word
                where
                    ...
                    aa = printBoard board foundWords 
...
printBoard :: Board -> [FoundWord] -> IO [()] -- TODO jakiś smiec sie tam jeszcze printuje
printBoard board foundWords = 
    ...
    where
        ...
        colorOutput c nr = "\x1b[" ++ show (30 + nr) ++ "m" ++ [c] ++ " "
